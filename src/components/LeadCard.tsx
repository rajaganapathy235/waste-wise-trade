...
export default function LeadCard({ lead }: { lead: Lead }) {
  ...
  const image = getMaterialImage(lead.materialType);

  return (
    <button
      onClick={() => navigate(`/lead/${lead.id}`)}
      className="w-full text-left bg-card rounded-lg border border-border overflow-hidden hover:shadow-md transition-all animate-fade-in"
    >
      {image && (
        <AspectRatio ratio={16 / 7}>
          <img
            src={image}
            alt={lead.materialType}
