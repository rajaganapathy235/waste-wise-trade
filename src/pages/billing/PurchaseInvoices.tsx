...
import { ArrowLeft, ShoppingCart, Search, FileDown, Plus, XCircle, Clock, CheckCircle2 } from "lucide-react";
...
export default function PurchaseInvoices() {
  ...
  const purchaseInvoices = invoices.filter(i => i.type === "purchase-invoice");
  let filtered = purchaseInvoices.filter(i => isInDateRange(i.date, dateRange));
  ...
  const totalPurchases = filtered.reduce((s, i) => s + i.totalAmount, 0);
  ...
  const handleExport = () => {
    ...
    exportToCSV("purchase_invoices.csv", headers, rows);
    ...
    <div className="px-4 pt-3 pb-8 max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold flex items-center gap-2"><ShoppingCart className="h-5 w-5 text-primary" /> Purchase Invoices</h1>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <Card className="border-l-4 border-l-primary">
          <CardContent className="p-3">
            ...
            <p className="text-sm font-bold">₹{totalPurchases.toLocaleString("en-IN")}</p>
          </CardContent>
        </Card>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto">
        {(["all", "paid", "unpaid", "partial"] as const).map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            ...
            {s === "all" ? `All (${purchaseInvoices.length})` : s}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No Purchase Invoices</p>
          <p className="text-[10px] text-muted-foreground mt-1">Create your first purchase invoice</p>
