    ...
    <div className="flex flex-col min-h-screen max-w-lg mx-auto bg-background">
      ...
      <div className="flex-1 px-4 pt-6 pb-8 space-y-6">
        ...
        <div
          ...
          onClick={() => navigate("/billing/tax-details", { state: { totalPrice: parseFloat(sellPrice) || 0, returnTo: `/billing/add-product${editId ? `?edit=${editId}` : ''}`, formData: { name, buyingPrice, sellPrice, mrp, unit, hsn } } })}
        >
          <div className="shrink-0 w-10 flex justify-center">
            <Film className="h-5 w-5 text-muted-foreground/40" />
          </div>
          <div className="flex-1 flex items-center border border-emerald rounded-md px-3 py-2">
            ...
            <span className="text-sm font-semibold text-foreground">
              {tax} % {parseFloat(sellPrice) ? `( ₹${((parseFloat(sellPrice) * (parseFloat(tax) || 0)) / 100).toFixed(2)} )` : ""}
            </span>
          </div>
        </div>
      </div>
