  ...
  return (
    <div className="px-4 pt-3 pb-8 max-w-md mx-auto">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-sm text-muted-foreground mb-4">
        <ArrowLeft className="h-4 w-4" /> {t("pulse.back")}
      </button>
      <div className="flex items-center gap-2 mb-1">
        <TrendingUp className="h-5 w-5 text-emerald" />
        <h1 className="text-lg font-bold">{t("pulse.title")}</h1>
      </div>
      <p className="text-xs text-muted-foreground mb-4">{t("pulse.subtitle")}</p>
      ...
      <div className="mb-4">
        <Select value={selectedMaterial} onValueChange={setSelectedMaterial}>
          ...
          <SelectContent>{MATERIAL_TYPES[activeCat].map((m) => (<SelectItem key={m} value={m}>{m}</SelectItem>))}</SelectContent>
        </Select>
      </div>

      <Card className="mb-4">
        <CardContent className="p-4">
          <p className="text-xs text-muted-foreground mb-2">{selectedMaterial} — {t("pulse.monthTrend")}</p>
      ...
      <h2 className="text-sm font-semibold mb-2">{activeCat} — {t("pulse.allMaterials")}</h2>
        ...
        {MATERIAL_TYPES[activeCat].map((mat) => {
          ...
          return (
            <button key={mat} onClick={() => setSelectedMaterial(mat)}
              className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${selectedMaterial === mat ? "border-primary bg-primary/5" : "border-border bg-card"}`}>
