...
export default function DemandHeatmap() {
  ...
  const filtered = activeCat === "All"
    ? mockDemandData
    : mockDemandData.filter((d) => d.category === activeCat);

  const sorted = [...filtered].sort((a, b) => b.searchCount - a.searchCount);
  const maxCount = sorted[0]?.searchCount || 1;

  // Aggregate by district for chart
  const districtMap: Record<string, number> = {};
  filtered.forEach((d) => {
    districtMap[d.district] = (districtMap[d.district] || 0) + d.searchCount;
  });
  ...
  return (
    <div className="px-4 pt-3 pb-8 max-w-md mx-auto">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-lg font-bold">Demand Heatmap</h1>
      </div>
      <p className="text-xs text-muted-foreground mb-4">Most searched materials in your region</p>
      ...
      <Card className="mb-4">
        <CardContent className="p-4">
          <p className="text-xs text-muted-foreground mb-2">Searches by District</p>
      ...
      {/* Material list */}
      <h2 className="text-sm font-semibold mb-2">Top Searched Materials</h2>
      <div className="space-y-2">
        {sorted.map((item, i) => (
          <div
            key={`${item.material}-${item.district}`}
            className={`flex items-center justify-between p-3 rounded-lg border ${getHeatColor(item.searchCount)}`}
          >
